...
Deno.test("sortPRs — empty list", () => {
  assertEquals(sortPRs([], "main"), [])
})

Deno.test("sortPRs — single PR with default branch base", () => {
  const prs = [makePR(1, "feat/a", "main")]
  assertEquals(sortPRs(prs, "main"), ["feat/a"])
})
...
Deno.test("sortPRs — linear chain", () => {
  ...
  assertEquals(sortPRs(prs, "main"), ["feat/a", "feat/b", "feat/c"])
})
...
Deno.test("sortPRs — fork (two branches based on same parent)", () => {
  ...
  const ordered = sortPRs(prs, "main")
...
Deno.test("sortPRs — no stack (all base on default)", () => {
  const prs = [
    makePR(1, "feat/a", "main"),
